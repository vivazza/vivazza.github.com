...
new ShareButton()
    ...
    // Thumbs overlay and gallery
    $(".grid-item").click(function() {
      image = $(this).html().replace(".jpg", "f.jpg");

      $(".overlay").fadeIn("slow", function() {
        $(this).append(image);
        $(this).children().addClass("full-screen").css("opacity", 1).fadeIn("slow");
        $(".overlay img").imagePanning();
      });
    });
    ...
    // Fade product images
    if ($(".overlay").is(":empty")) {
      $(".grid-item img").mouseover(function() {
        $(this).animate({
          opacity: .6
        }, 250)
      }).mouseout(function() {
    ...
    // Image Panning by http://manos.malihu.gr/jquery-image-panning/
	...
	$(document).mouseleave(function(){
			...
			$(".pop-overlay").fadeIn("slow");
			$(".pop-inner").fadeIn("slow");
			Cookies.set("pop", "served", { expires: 1 });
		};

   });

	$(".pop-submit").click(function() {
		Cookies.set("pop", "subscribed");
	})

	$(".no-thanks").click(function() {
		$(".pop-overlay").fadeOut("slow");
	    $(".pop-inner").fadeOut("slow");
		Cookies.set("pop", "nothanks", { expires: 1 });
	})
